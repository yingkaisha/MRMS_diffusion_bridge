...
if load_num >= 0:
    model_name_load = '/glade/work/ksha/GAN/models/DB_elev128_tune{}/'.format(load_num)
else:
    model_name_load = '/glade/work/ksha/GAN/models/DB_elev128/'

model_name_save = '/glade/work/ksha/GAN/models/DB_elev128_tune{}/'.format(save_num)
...
for i in range(epochs):
    ...
    for j in range(L_train):
        ...
        for k, ind in enumerate(inds_):
            ...
            Y_batch_HR[k, ..., 1] = temp_data[..., -1]
            Y_batch_HR[k, ..., 0] = precip_norm(temp_data[..., 0], max_apcp)
        ...
        noise = np.random.normal(size=(batch_size, img_size, img_size, 1))
        images_t = np.array(gdf_util.q_sample(Y_batch_HR[..., 0][..., None], t, noise))
        model.train_on_batch([np.concatenate((images_t, Y_batch_HR[..., 1:]), axis=-1), t], noise)

    print('Save model: {}'.format(model_name_save))
