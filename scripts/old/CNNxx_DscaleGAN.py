...
model_de_save = 'DscaleGAN_de{}'.format(train_round)
model_gen_save = 'DscaleGAN_gen{}'.format(train_round)

model_de_load = 'DscaleGAN_de{}'.format(weights_round)
model_gen_load = 'DscaleGAN_gen{}'.format(weights_round)
...
def Mapping(num_stages, latent_shape=60):
    '''
    The mapping network
    ...
    for i in range(8):
        w = EqualizedDense(latent_shape, learning_rate_multiplier=0.01)(w)
...
def generator(size_input_res, size_input_channel, size_latent_channel, filter_nums, size_progress):
    ...
    out = x
    out = layers.Conv2D(1, 1, padding="valid")(out)
...
def discriminator(size_output_res, size_output_channel):
    ...
    x = EqualizedConv(16, 1)(x)
    x = layers.LeakyReLU(0.2)(x)
    x = EqualizedConv(16, 3)(x)
    x = layers.LeakyReLU(0.2)(x)
