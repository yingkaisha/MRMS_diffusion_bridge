...
batch_file_name = 'baseline_y{:04d}_ini{:02d}_lead{:02d}_dt{:04d}_ix{:03d}_iy{:03d}.npy'
                                ...
                                if (ix_end < grid_shape[0]) and (iy_end < grid_shape[1]):
                                    ...
                                    if np.sum(temp_mrms_flag > V_rain_thres) > N_rain_thres:
                                        ...
                                        if np.sum(np.isnan(data)) == 0:
                                            ...
                                            Y_latent_gfs = model_encoder_mrms.predict(data_temp[..., 1][..., None], verbose=0)

                                            data_save = {}
                                            data_save['Y_latent'] = Y_latent
                                            data_save['Y_latent_gfs'] = Y_latent_gfs
